...
Another possible issue that could happen is that the `coredns` pods are not running or not ready. The firewall needs to be properly checked to allow the ports that Kubernetes uses through the firewall. Alternatively, the firewall can be disabled completely.
